...
const PropertyDetails = ({ userInfo }) => {
  ...
  const [isLikePending, setIsLikePending] = useState(false); // Add state to track like request pending status
...
const togglePostLike = async () => {
  if (isLikePending) return; // Prevent multiple like requests while one is pending
  setIsLikePending(true); // Set like request pending status
  ...
  try {
    ...
    // Assuming successful response means like status updated, no need to wait for data fetch
    setLike(!like);

  } catch (error) {
    console.log(error);
  } finally {
    setIsLikePending(false); // Reset like request pending status after request completion
  }
};
console.log(data)
  return (
  ...
  {like ? (
    <>
      <button
        ...
        className="flex items-center justify-between gap-2 font-bold border px-6 py-2 rounded-lg border-black"
        disabled={isLikePending} // Disable button while like request is pending
      >
        <svg
